defmodule Dujudu.Access.IngredientsTest do
  ...
  describe "sample_ingredients/2" do
    setup do
      ...
      %{
        ingredients: %{
          ...
          with_images: insert(:ingredient, commons_image_urls: ["booourl", "schnourl"], wikidata_id: "Q124"),
    ...
    test "limits result by parameter" do
      _extra_ingredient = insert(:ingredient, commons_image_urls: ["blargel"], wikidata_id: "Q126")
    ...
    test "provides deterministic random order based on seed" do
      ...
      assert [["fooooourl"], ["booourl", "schnourl"]] == result1 |> Enum.map(& &1.commons_image_urls)
      assert [["booourl", "schnourl"], ["fooooourl"]] == result2 |> Enum.map(& &1.commons_image_urls)
    end
  end
end
